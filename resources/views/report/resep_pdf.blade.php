...
<html>
<head>
    <title>Laporan Permintaan Resep</title>
    <style>
        @page { margin: 1cm; }
        body { 
...
<body>
    ...
    <div class="document-info">
        <table>
            <tr>
                ...
                <td width="20">:</td>
                <td>{{ sprintf('%03d', rand(100, 999)) }}/Lap-Resp/RSUN/{{ date('m/Y') }}</td>
            </tr>
            <tr>
                <td><strong>Perihal</strong></td>
                <td>:</td>
                <td><strong>Laporan Permintaan Resep Obat</strong></td>
            </tr>
        </table>
    </div>
    
    <div class="header">
        <h2>REKAP PERMINTAAN RESEP OBAT</h2>
    ...
    <table>
        <thead>
            <tr>
                <th width="15%">No. Resep / Tgl</th>
                <th width="20%">Pasien / RM</th>
                <th width="65%">Daftar Permintaan Obat</th>
            </tr>
        </thead>
        <tbody>
            @foreach($laporan as $row)
            <tr>
                <td style="text-align:center;"><strong>{{ $row->no_resep }}</strong><br>{{ $row->tgl_peresepan }}</td>
                <td><strong>{{ $row->regPeriksa->pasien->nm_pasien }}</strong><br>RM: {{ $row->regPeriksa->no_rkm_medis }}</td>
                <td>
                        ...
                        <div style="margin-bottom: 5px;">
                            ...
                            @php
                                echo $row->resepDokter->map(function($item) {
                                    return $item->dataBarang->nama_brng . " (" . $item->jml . ")";
                                })->implode('; ');
                            @endphp
                        </div>
                    @endif
