    ...
    <div class="document-info">
        <table>
            <tr>
                ...
                <td width="20">:</td>
                <td>{{ sprintf('%03d', rand(100, 999)) }}/Lap-Rad/RSUN/{{ date('m/Y') }}</td>
            </tr>
            <tr>
                <td><strong>Perihal</strong></td>
                <td>:</td>
                <td><strong>Laporan Permintaan Radiologi</strong></td>
            </tr>
        </table>
    </div>
    
    <div class="header">
        <h2>REKAP PERMINTAAN PEMERIKSAAN RADIOLOGI</h2>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th width="40%">Diagnosa Klinis & Info</th>
            </tr>
        </thead>
        <tbody>
                ...
                <tr>
                    @if($index === 0)
                        <td rowspan="{{ $rowCount }}" style="text-align:center;">
                            <strong>{{ $row->noorder }}</strong><br>{{ $row->tgl_permintaan }}
                        </td>
                        <td rowspan="{{ $rowCount }}">
                    ...
                    @if($index === 0)
                        <td rowspan="{{ $rowCount }}">
                            ...
                            <strong>Info:</strong> {{ $row->informasi_tambahan }}
                        </td>
                    @endif
                </tr>
                @endforeach
